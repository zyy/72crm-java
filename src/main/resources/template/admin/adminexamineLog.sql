#namespace("admin.examineLog")
  ...
  #sql("queryExamineLogByRecordId")
    select sael.* ,(SELECT realname from 72crm_admin_user WHERE user_id = sael.create_user) as createUserName,(SELECT realname from 72crm_admin_user WHERE user_id = sael.examine_user) as examineUserName
  ...
  #sql("queryUserByUserId")
    SELECT DISTINCT saud.user_id, saud.realname , 0 as examine_status from 72crm_admin_user as sau LEFT JOIN 72crm_admin_user as saud on saud.user_id = sau.parent_id
    WHERE sau.user_id = ?
  #end
  ...
  #sql("queryUserByRecordIdAndStepIdAndStatus")
    select sael.examine_time  as examineTime , sael.examine_status,sau.realname as realname , sau.user_id , sau.img from 72crm_admin_examine_log as sael LEFT JOIN 72crm_admin_user as sau on sau.user_id = sael.examine_user
  ...
  select sael.examine_time  as examineTime , sael.examine_status,sau.realname ,sau.img , sael.log_id from 72crm_admin_examine_log as sael LEFT JOIN 72crm_admin_user as sau on sau.user_id = sael.examine_user
  where sael.record_id = ? and sael.is_recheck != 1 order by sael.create_time
  #end
  #sql("queryExamineLogAndUserByLogId")
  select sael.examine_time  as examineTime , sael.examine_status,sau.realname ,sau.img , sael.log_id from 72crm_admin_examine_log as sael LEFT JOIN 72crm_admin_user as sau on sau.user_id = sael.examine_user where sael.log_id = ? and sael.is_recheck != 1 order by sael.create_time
  #end
  #sql("queryExamineLog")
  ...
  #sql("queryRecordAndId")
    select create_user, sau.realname as examineUserName , sau.user_id , sau.img, aser.create_time as examineTime, 5 as examine_status from 72crm_admin_examine_record  as aser LEFT JOIN 72crm_admin_user as sau on sau.user_id = aser.create_user where record_id = ?
  #end
  ...
  #sql("queryNowadayExamineLogByRecordIdAndStatus")
    select * from 72crm_admin_examine_log where record_id = ? and examine_status = 0 and examine_user = ? and is_recheck = 0
  #end
