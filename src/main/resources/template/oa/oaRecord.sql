#namespace("oa.record")
  #sql("queryList")
    select a.log_id,a.action_id,a.content as action_content,a.create_time,a.type,a.user_id from 72crm_oa_action_record a  where 1 = 1 and
      case when type = 2 or type = 3 or type = 4 then  (a.join_user_ids like concat('%,',#para(userId),',%') or a.dept_ids like concat('%,',#para(deptId),',%') or a.user_id = #para(userId))
      else   (
                #for(i : userIds)
                 #(for.index > 0 ? "or" : "")
                ...
                #end
                #for(i : deptIds)
                     or a.dept_ids like concat('%,',#para(i),',%')
                #end
